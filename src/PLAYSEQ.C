...
//    InitDebug();
    ...
    ***********************************************/
	...
	//в этом сегменте смещаем тональность ^^^^^^^
    /**********************************************
...
//			EPrint("EOF %d \n",tnote);
		...
		//Print("SP:%d n:%d d:%d\n",pos,note,dur);
	  	...
	  	npos = (( tpos / kcnf.base_kara ) + 1) * kcnf.base_kara; //количество тиков миди на сильную долю караоке
		//Print("NPos1:%d\n",npos);
		...
		while ( npos<(tpos+tdur))
		{
			...
			tdur -= cheap; 
			tpos = npos;
			//cheap = kcnf.base_kara; //!!! проверить
			npos+= kcnf.base_kara;
		}
		
		note_write(tnote,tpos,tdur);		// !!! проверить tdur==0 writen previous note 
